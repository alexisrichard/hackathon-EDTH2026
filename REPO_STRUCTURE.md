...
│   ├── ais|sar|optical/ #   local mirrors of S3 big data — GITIGNORED (pull via scripts)
│   └── samples/notebooks/   starter notebook
│
├── scripts/             # ░ DATA lane ░ data-prep pipelines (mostly complete — stable)
│   ├── common/          #   shared helpers: bbox, DuckDB conn, S3 sync  ← import these everywhere
...
│   │   └── data/        #   DuckDB-over-S3 query layer (reads ais/ parquet, geo layers)
│   └── README.md
│
...
|---|---|---|---|---|
| **Data & Backend** | _Alexis / TBD_ | `backend/`, `scripts/`, `data/` | S3/DuckDB access, FastAPI serving tracks + scores + cues, scenario replay endpoints | `scoring.score`, `shared/api_contract.md` |
...
The data-prep work (`scripts/`, S3) is largely done — that lane's real day-of job is the **backend**. ML and Frontend are net-new.
...
| A query that reads AIS parquet from S3 | `backend/app/data/` (or `scripts/common/` if broadly reusable) |
...
| A big derived file (parquet, GeoTIFF, >10 MB) | S3 (`s3://edth2026-baltic/…`), **not** git — see `.gitignore` |
